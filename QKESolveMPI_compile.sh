...
echo "mpiexec -n $numprocs wed $xmin $xmax $numlin $numgl $eta_e $eta_mu $sin2theta $deltamsquared $N_step $dN $x_initial $x_final $dx_initial $verbose \"$output_file\" \"$input_file\" " >> QKESolveMPI_execute.sh