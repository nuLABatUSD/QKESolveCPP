...
nu_nu_collision_one_1::nu_nu_collision_one_1(linspace_and_gl* e, int p1_index){
    eps = new linspace_and_gl(e);
    p1 = p1_index;
    count = 0;
    outer_vals = new dep_vars(eps->get_len());
...
void nu_nu_collision_one_1::Fvvsc_components_term_1(density* dens, bool neutrino, int p2, int p3, double* F0, three_vector* F){
    ...
    if (eps->get_value(p1)<=max_lin and eps->get_value(p2)<=max_lin and p3_energy<=max_lin and p4_energy<=max_lin){
        ...
        three_vector* A = new three_vector();
        double A0 = dens->interpolated_matrix(neutrino, p4_energy, A);
        p_4->convert_p_to_matrix(A0, A);
        delete A;
    }
    /*
...
void nu_nu_collision_one_1::Fvvsc_components_term_2(density* dens, bool neutrino, int p2, int p3, double* F0, three_vector* F){
...
void nu_nu_collision_one_1::Fvvsc_components(density* dens, bool neutrino, int p2, int p3, double* F03, three_vector* F3){
...
void nu_nu_collision_one_1::Fvvsc_for_p1(density* dens, bool neutrino){
    double F0 = 0;
    three_vector* Fxyz = new three_vector();
    for(int p2=0; p2<eps->get_len(); p2++){
        for(int p3=0; p3<p3_vals[p2]->get_len(); p3++){
...
void nu_nu_collision_one_1::Fvvbarsc_components_term_1(density* dens, bool neutrino, int p2, int p3, double* F0, three_vector* F){
    matrix* p_1 = new matrix();
    matrix* p_2 = new matrix();
    matrix* p_3 = new matrix();
    ...
    if (eps->get_value(p1)<=max_lin and eps->get_value(p2)<=max_lin and eps->get_value(p3)<=max_lin and p4_energy<=max_lin){
        ...
        three_vector* A = new three_vector();
        double A0 = dens->interpolated_matrix(not neutrino, p4_energy, A);
        p_4->convert_p_to_matrix(A0, A);
        delete A;
    }
...
void nu_nu_collision_one_1::Fvvbarsc_components_term_2(density* dens, bool neutrino, int p2, int p3, double* F0, three_vector* F){
    matrix* p_1 = new matrix();
    matrix* p_2 = new matrix();
    matrix* p_3 = new matrix();
...
void nu_nu_collision_one_1::Fvvbarsc_components(density* dens, bool neutrino, int p2, int p3, double* F03, three_vector* F3){
...
void nu_nu_collision_one_1::Fvvbarsc_for_p1(density* dens, bool neutrino){
    double F0 = 0;
    three_vector* Fxyz = new three_vector();
    for(int p2=0; p2<eps->get_len(); p2++){
        for(int p3=0; p3<p3_vals[p2]->get_len(); p3++){
...
double nu_nu_collision_one_1::J1(double p1, double p2, double p3){
    return 16./15 * pow(p3,3) * (10 * pow(p1+p2,2) - 15 * (p1+p2) * p3 + 6*pow(p3,2));  
}

double nu_nu_collision_one_1::J2(double p1, double p2){
    return 16./15 * pow(p2,3) * (10 * pow(p1,2) + 5 * p1*p2 + pow(p2,2));  
}

double nu_nu_collision_one_1::J3(double p1, double p2, double p3){
    return 16./15 * (pow(p1+p2,5) - 10 * pow(p1+p2,2) * pow(p3, 3) + 15 * (p1+p2) * pow(p3,4) - 6 * pow(p3,5));
}

double nu_nu_collision_one_1::K1(double p1, double p3){
    return 16./15 * pow(p3,3) * (10 * pow(p1,2) - 5 * p1*p3 + pow(p3,2));
}

double nu_nu_collision_one_1::K2(double p1, double p2, double p3){
    return 16./15 * pow(p2,3) * (10 * pow(p1-p3,2) + 15 * (p1-p3) * p2 + 6 * pow(p2,2));
}

double nu_nu_collision_one_1::K3(double p1, double p2, double p3){
    return 16./15 * (pow(p1-p3,5) + 10 * pow(p1-p3,2) * pow(p2,3) + 15 * (p1-p3) * pow(p2,4) + 6 * pow(p2,5));
}


double nu_nu_collision_one_1::interior_integral(int p2, int which_term){
    double p_1_energy = eps->get_value(p1);
    double max_energy = eps->get_value(eps->get_len()-1);
...
//note: results must be length 4
void nu_nu_collision_one_1::whole_integral(density* dens, bool neutrino, double* results){
    if (p1==0){
        for(int i=0; i<4; i++){
            results[i] = 0;
        }
...
nu_nu_collision_one_1::~nu_nu_collision_one_1(){
    delete outer_vals;
    for(int i=0; i<eps->get_len(); i++){
        delete inner_vals[i];
        delete p3_vals[i];
